...
const convertDate = (date) => {     //! Ya la utilice en muchos reportes
    if( !date ) return ''
...
const pdfTableHead = [
    [
        { content: '', colSpan: 1 },
        { content: 'Despacho', colSpan: 10 },
        { content: 'Recepción', colSpan: 11, styles: { lineWidth: { top: 0.25, right: 0.25, bottom: 0.25, left: 1.0  }} },
    ],
...
export const getReporteEcosBitacora = async({ 
    ...
    let consultaComplemento = '&solo_despachos=1&complemento=1';
    ...
    if(tipo?.length==1 && tipo[0] === 1)  {
        ...
        consultaComplemento  = '&solo_despachos=1';
        consultaTipo=1
        pdfTablaCols = cols
        format = 'letter'
        head = undefined
    ...
    if(tipo?.length==1 && tipo[0] === 2)  {
        pdfTituloTipo = 'RECEPCIONES';
        consultaComplemento  = '&solo_recepciones=1';
        consultaTipo=2
        pdfTablaCols = cols
        format = 'letter'
        head = undefined
    ...
    if(!!cred)    consultaComplemento += `&cred=${cred}`;
    if(!!ruta)    consultaComplemento += `&ruta=${ruta}`;
    if(!!motivo)  consultaComplemento += `&motivo=${motivo}`;


    //& Get data (Realizar consulta)
    let res: EcosDespachadosConRecepcion[] = []
    try {
        res = await UseFetchGet(`${API}/api/caseta/ecosBitacora?fecha_ini=${fechaIni}&fecha_fin=${fechaFin}&modulo=${modulo}${consultaComplemento}`)
    } catch (error) {
    ...
    const data = res.map( (reg, i) => {
        const data = {
            ...
            time1:       convertDate(reg.modulo_time),
            op_turno1:   reg.turno,
        ...
        if(consultaTipo === 3){
            complemento = {
                modulo2:     reg.complemento?.modulo_id,
                cap2:        reg.complemento?.cap_user,
                eco2:        reg.complemento?.eco,
                op_cred2:    reg.complemento?.op_cred,
                time2:       convertDate(reg.complemento?.modulo_time),
                op_turno2:   reg.complemento?.turno,
                extintor2:   reg.complemento?.extintor,
                ruta2:       reg.complemento?.ruta_swap,
                modalidad2:  reg.complemento?.ruta_modalidad,
                cc2:         reg.complemento?.ruta_cc,
                motivo2:     reg.complemento?.motivo,
    ...
    const makePDF = (dataTable) => {
        // const { bloburl } = exportTablePDF( data, pdfTablaCols, { 
        const { bloburl } = exportTablePDF( dataTable, pdfTablaCols, { 
            ...
            tableOptions: {
                columnStyles: { 
                    ...
                    modulo2:  { lineWidth: { top: 0.25, right: 0.25, bottom: 0.25, left: 1.0 } }, 
                },
                head ,
                tableLineColor: '#000',
                tableLineWidth: 1,

...
const convertDate = (inputDate) => {  
  ...
  if( inputDate.length > 1 ){    
    inputDate.map( (fecha) => {
      ...
      inputDate.push( `${year}/${month}/${date}` )
    } )
...
export const FormMaker = ({ 
  ...
  newProps=[], 
  defaultValues={},   // Chin%&*$umadre! perra dislexia (era defaultValues)
  forButton, 
  boton2 
}) => {

  const { handleSubmit, control, formState:{errors}, setValue, getValues, reset, watch, resetField, setError, clearErrors } = useForm({defaultValues})

  //#! No seria mejor esperar los JSX.Element[] ?
  //& Personalizacion del botón, si es que es pasado un objeto, con ayuda de PrimeFaces se ajusta la posición del boton (left, center, right)
  const [boton, setBoton] = useState( { label:"Guardar", posicion: 'text-center', estilo:'', icon:"pi pi-check", iconPosicion:"left" } )
  useEffect(() => {
    if ( Boolean(forButton) ){
      Boolean(forButton.posicion) && ( forButton.posicion = `text-${forButton.posicion}` );
      setBoton( (botonDefault) => ({...botonDefault, ...forButton}) )
    } 
  }, [])  

  //& variables usadas en myHandleSubmit
  let calendars = [];   // Inputs a aplicar convertDate()
  let selects = [];     
  let unReginputs = []; // Inputs a quitar del submit.

  //& Tener las funciones "setValue" & "getValues" fuera del componente
  useEffect(() => {
  ...
  //#! ¿? Porque esta esto aqui ??? No se puede pasar directamente al FormItem para, si existen, ejecutar las funciones?
  ...
  const forOnBlur = (e,setValue, getValues, callBackForOnBlur='', resetField, setError, clearErrors) => {
  ...
  const forOnChange = (e, setValue, getValues, callBackForOnChange='') => {
    if ( Boolean(callBackForOnChange) ) 
      callBackForOnChange( {e, setValue, getValues} )
  }


  //& myHandleSubmit. No retorna nada ya que se espera que se maneje con callBackForSubmit
  const myHandleSubmit = (data) => {
    ...
    //? De todos los inputType == select que su valor sea el del code
    selects.map( (id) => {
      ...
      <div className="card formgrid grid my-2 w-full">
      {
        inputs.map( (input, i) => {
          ...
          <FormItem2 
            ...
            value={input.value} 
            info={input.info} 
            bigInfo={input.bigInfo} 
            canSelectAll={input.canSelectAll} 
            callBackForOnBlur={input.callBackForOnBlur} 
            validaciones={input.validaciones} 
            disabled={input.disabled} 
            selectionModeForCal={input.selectionModeForCal} 
            inputType={input.inputType} 
            options={input.optionsforSelect}  
            callBackForOnChange={input.callBackForOnChange} 
            callBackForRender={input.callBackForRender} 
        ...
        />
        {
          Boolean(boton2) && 
            <Button 
              label={boton2.label} 
              icon={boton2.icon} 
              iconPos={boton2.iconPosicion || 'left'} 
              type="button"
              className={`ml-8 p-button-${Boolean(boton2.estilo) ? boton2.estilo: ''}`} 

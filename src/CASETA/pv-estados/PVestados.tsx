...
import { useReportesGacetaStore } from "../../shared/store/useReportesGaceta.store";
...
const OPTS_MODS_PUERTAS = {
  ...
  4: [
    { name: "Todas", value: 0 },
    { name: "M04-Minas", value: "M04-Minas" },
    { name: "M04-Alfa", value: "M04-Alfa" },
  ],
  5: [{ name: "M05", value: "M05" }],
  6: [
    { name: "Todas", value: 0 },
    { name: "M06", value: "M06" },
    { name: "M06-Anexo", value: "M06-Anexo" },
    { name: "M06-46", value: "M06-46" },
  ],
  7: [
    { name: "Todas", value: 0 },
    { name: "M07", value: "M07" },
    { name: "M07-Anexo", value: "M07-Anexo" },
    { name: "M07-33", value: "M07-33" },
  ],
      ...
      <div className="flex flex-wrap justify-content-between align-items-stretch gap-3">
        {userModulo === 0 ? (
          <div className="flex-center gap-3">
            ...
            {OPTS_MODS_PUERTAS[modulo] &&
              OPTS_MODS_PUERTAS[modulo].length > 1 && (
                <span className="p-float-label mt-3 w-8rem">
                  <Dropdown
                    inputId="modulo_puerta"
                    value={puerta}
                    onChange={(e) => setPuerta(e.value)}
                    options={OPTS_MODS_PUERTAS[modulo]}
                    optionLabel="name"
                    className="w-full"
                  />
...
const onFormSubmit = (e: SubmitState) => {
  ...
  const data: Data2PostNewEcoState = {
    momento: new Date(
      d.time_fecha.split("/").reverse().join("-") + " " + d.time_hora,
    ),
    tipo: d.regTipo,
    eco: Number(d.eco),
    eco_tipo: d.eco_tipo,
    ...
    motivo_desc: !!d.dynamicInputs
      ? JSON.stringify(d.dynamicInputs)
      : undefined,
    ...
    reg_previo: regPrev
      ? { id: regPrev.id, tipo: regPrev.tipo as 1 | 2 | 3 | 4 }
      : undefined,
  ...
  UseFetchPost(`${API}/api/caseta/pv-estados`, data)
    .then((res) => {
      ...
      toast.current?.show({
        severity: "success",
        life: 5000,
        summary: `Eco ${d.eco} tiene un nuevo estado!`,
      });
      resetForm();
    ...
    .catch((err) => {
      ...
      toast.current?.show({
        severity: "error",
        summary: `Error al registrar el eco ${d.eco}`,
        sticky: true,
      });
    })
...
const DialogReportes = () => {
  ...
  const visible = useReportesGacetaStore((s) => s.visible);
  const close = useReportesGacetaStore((s) => s.close);
  ...
  useEffect(() => {
    setTimeout(() => {
      panel &&
        removeHTMLelements("#reporte-panel", ".p-column-filter-clear-button");
    }, 200);
  }, [panel]);
  ...
  return (
    <div className="card flex justify-content-center">
      <Button
        label="Reportes"
        severity="secondary"
        onClick={useReportesGacetaStore.getState().open}
      />
      {/* <Dialog header="Header" visible={visible} style={{ width: '50vw' }} onHide={() => setVisible(false)}> */}
      <Dialog
        id="caseta-dialog-reportes"
        modal
        maximizable
        position="right"
        visible={visible}
      ...
      >
        ...
        <div
          className="my-3 mx-auto w-12"
          style={{ borderBottom: "2px solid #7BC179" }}
        />

        <div id="reporte-panel">{paneles[panel]}</div>
      </Dialog>
    </div>
  );
};
...
const ReportePVestados = () => {
  ...
  const fechas = useWatch({
    control: form.control,
    name: ["fecha_ini", "fecha_fin"],
  });
  ...
  const onSubmit = async (d: Form) => {
    ...
    try {
      const resp: FetchPVestados = await UseFetchGet(
        `${API}/api/caseta/pv-estados?create_modulo=${userModulo}&${query}`,
      );
      ...
      setTable({
        data: resp.results.map((reg) => ({
          ...
          eco_estatus: { 1: "DISPONIBLE", 2: "NO DISPONIBLE" }[
            reg.eco_estatus
          ] as any,
  ...
  useEffect(() => {
    ((fechas[0] && !fechas[1]) || fechas[0] > fechas[1]) &&
      form.setValue("fecha_fin", fechas[0]);
  }, [fechas[0]]);
    ...
    <>
        ...
        <MiniFormMaker
          ...
          inputs={[
            ...
            { name: "eco", label: "Eco*", keyfilter: "money", required: true },
            {
              ...
              label: "Fecha inicial",
              type: "calendar",
              tooltip: "Mayor al 25 de Marzo de 2024",
              tooltipOptions: { position: "bottom" },
              minDate: new Date("2024-03-25T06:00:00.000Z"),
              showButtonBar: true,
            },
            {
              name: "fecha_fin",
      ...
      {table.show ? (
        !table.error ? (
          ...
          <h3 className="bg-pink-100 py-2 text-center capitalize">
            {table.error}
          </h3>
        )
      ) : (

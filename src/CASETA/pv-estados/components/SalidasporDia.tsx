...
import {
  ...
  turnos,
  getTurnoOptions,
} from "../utilities/turnosUtils";
import {
  mostrarTotalesEnBarra,
  mostrarTotalesEnLinea,
} from "../utilities/chartPlugins";
...
export const SalidasporDia = () => {
  ...
  const [data, setData] = useState<
    (TableData & { id: number; detalles?: any[] })[]
  >([]);
  ...
  const [filtrosActuales, setFiltrosActuales] = useState<FormFilters | null>(
    null,
  );
  ...
  const onSubmit = async (formData: FormFilters) => {
    ...
    try {
      ...
      const filtrados = res.data.filter(
        (reg) =>
          reg.motivo1 === "SERVICIO" &&
          reg.modalidad1 !== "METROBÚS" &&
          reg.modalidad1 !== "NOCHEBÚS",
      );
      ...
      const turnoSeleccionado = String(
        form.getValues("turnoFiltro") || turnoFiltro,
      );
      if (formData.modulo === 0) {
        ...
        dataAgrupada = modulosBase.map((modulo, idx) => {
          ...
          const detallesFiltrados = filtrarPorTurno(
            registros,
            turnoSeleccionado,
          );
          return {
        ...
        dataAgrupada = Object.entries(agrupado).map(
          ([ruta, registros], idx) => {
            const detallesFiltrados = filtrarPorTurno(
              registros,
              turnoSeleccionado,
  ...
  return (
    <div className="card">
      <h2 className="text-center">Salidas por Día</h2>

      {/* Formulario de filtros */}
      <form
        onSubmit={form.handleSubmit(onSubmit)}
        className="flex-center gap-3 mt-5"
      >
        <MiniFormMaker
          form={form}
          inputs={[
              ...
              ? [
                  {
                    ...
                    options: getTurnoOptionsMemo().map((opt) => ({
                      ...opt,
                      name: opt.label,
                    })),
                    value: turnoFiltro,
          ...
          <>
            ...
            <h3 className="text-center mb-4">
              {(() => {
                ...
                const fechaIni = filtrosActuales.fecha_ini
                  ? filtrosActuales.fecha_ini.toLocaleDateString()
                  : "";
                ...
                const moduloObj = modulos.find(
                  (m) => m.value === moduloSeleccionado,
                );
                const moduloTexto = moduloObj ? moduloObj.value : "";
                const fechaFin = filtrosActuales.fecha_fin
                  ? filtrosActuales.fecha_fin.toLocaleDateString()
                  : "";
            ...
            >
                  ...
                  <Chart
                      ...
                      datasets: [
                        {
                          ...
                          data: [1, 2, 3, 4, 5, 6, 7].map(
                            (modulo) =>
                              filtrarPorTurno(
                                filtradosData.filter(
                                  (reg: any) => Number(reg.modulo) === modulo,
                                ),
                                String(
                                  filtrosActuales?.turnoFiltro || turnoFiltro,
                                ),
                              ).length,
                          ),
                    ...
                    options={{
                      ...
                      scales: {
                        ...
                        y: {
                          title: { display: true, text: "Total de Económicos" },
                          beginAtZero: true,
                        },
                      },
              ...
              {(() => {
                ...
                if (
                  filtrosActuales &&
                  ["t1", "t2", "t3"].includes(filtrosActuales.turnoFiltro || "")
                ) {
                  if (filtrosActuales.turnoFiltro === "t1")
                    turnosToShow = ["t1"];
                  if (filtrosActuales.turnoFiltro === "t2")
                    turnosToShow = ["t1", "t2"];
                  if (filtrosActuales.turnoFiltro === "t3")
                    turnosToShow = ["t1", "t2", "t3"];
                } else if (
                  filtrosActuales &&
                  filtrosActuales.turnoFiltro === "dia"
                ) {
                  turnosToShow = ["t1", "t2", "t3"];
                } else if (
                  filtrosActuales &&
                  filtrosActuales.turnoFiltro === "hasta"
                ) {
                  ...
                  label:
                    turno === "t1"
                      ...
                      : turno === "t2"
                        ...
                        : "Tercer turno",
                  data: rutas.map(
                    (ruta) => filtrarPorTurno(agrupado[ruta], turno).length,
                  ),
            ...
            {((filtrosActuales?.modulo === 0 && moduloSeleccionado !== null) ||
              filtrosActuales?.modulo !== 0) &&
              ["t1", "t2", "t3", "dia", "hasta"].includes(
                filtrosActuales?.turnoFiltro || "",
              ) && (
                <div
                  style={{
                    display: "flex",
                ...
                >
                  {(() => {
                    ...
                    return turnosToShow.map((turno) => {
                      ...
                      const turnoLabel =
                        turno === "t1"
                          ? "Primer turno (4:59am - 10:00am)"
                          : turno === "t2"
                            ? "Segundo turno (4:59am - 5:00pm)"
                            : "Tercer turno (4:59am - 8:00pm)";
                      const color =
                        turno === "t1"
                          ? "#66BB6A"
                          : turno === "t2"
                        ...
                        >
                          ...
                          >
                            {(() => {
                              ...
                              const registrosModulo = filtradosData.filter(
                                (reg: any) =>
                                  Number(reg.modulo) === Number(moduloActual),
                              );
                              const totalTurno = filtrarPorTurno(
                                registrosModulo,
                                turno,
                              ).length;
            ...
            {(() => {
              ...
              if (filtrosActuales) {
                ...
                if (filtrosActuales.modulo === 0) {
                  totalSalidas = data.reduce(
                    (acc, curr) =>
                      acc +
                      (Array.isArray(curr.detalles) ? curr.detalles.length : 0),
                    0,
                  );
                } else {
              ...
              let columnasModulo: any[] = [
                ...
                { field: "ruta", header: "Rutas" },
                {
                  ...
                  body: (rowData: any) => {
                    const rutaObj = OPTS_RUTA?.find(
                      (r: any) => r.value?.nombre === rowData.ruta,
                    );
                    return rutaObj
                  ...
                  body: (rowData: any) => (
                    ...
                    >
                      ...
                      <Button
                        ...
                        onClick={() =>
                          handleVerDetalles(rowData[`detalles_${turno}`] ?? [])
                        }
                      />
                    </span>
                  ),
              ...
              if (filtrosActuales?.modulo === 0) {
                ...
                const tablaGeneral = modulosBase.map((modulo, idx) => {
                  const registros = filtradosData.filter(
                    (reg: any) => Number(reg.modulo) === modulo,
                  );
                  return {
                    id: idx + 1,
                ...
                // Totales generales
                const total_t1 = tablaGeneral.reduce(
                  (acc, curr) => acc + curr.total_t1,
                  0,
                );
                const total_t2 = tablaGeneral.reduce(
                  (acc, curr) => acc + curr.total_t2,
                  0,
                );
                const total_t3 = tablaGeneral.reduce(
                  (acc, curr) => acc + curr.total_t3,
                  0,
                );
                ...
                let dataModulo = data.map((row) => {
                  ...
                  turnosToShow.forEach((turno) => {
                    rowTurnos[`total_${turno}`] = filtrarPorTurno(
                      row.detalles,
                      turno,
                    ).length;
                    rowTurnos[`detalles_${turno}`] = filtrarPorTurno(
                      row.detalles,
                      turno,
                    );
                  });

...
export const getReporteExtintores = async(params: Params) => {
    ...
    try {
        res = await UseFetchGet(`${API}/api/caseta/ecosBitacora?fecha_ini=${fechaIni}&fecha_fin=${fechaFin}&modulo=${modulo}&solo_despachos=1&complemento=1`)
    } catch (error) {
    ...
    const data = res.map( (despacho, i) => {
        ...
        return {
            ...
            time_in:     convertDate(recepcion?.modulo_time),
